...
The pipeline itself is written in a mix of Groovy, Python and bash, the Code and a flowchart describing the process can be found in the appendix at \ref{appendix:cicd_pipeline}.
...
The image also contains the necessary tools to interact with the Pahcyderm API and to run the python scripts which are used to analyze the project structure and do the code generation, it can be found at \ref{appendix:jenkins_dockerfile}.

While this pipeline is currently working as intended, it is still missing many edgcases and does need further development to be able to handle more thant the most basic projects.
